...
Typograf.addRule({
  ...
  handler: (text) =>
    text
      ...
      .replace(/([^\s])(\[|\()/g, '$1 $2')
      // Добавляем пробел после закрывающей скобки, если после нее нет пробела, знака препинания или конца строки
      .replace(/([\]\)])(?![\s.,!?;:)])(?=\S)/g, '$1 '),
});
...
// Выключаем правило замены "/" на "|"
typograf.disableRule('common/punctuation/replaceSlashWithPipe');
...
const config = [
  ...
  { triggerSelector: '[for="Lesson_title"]', targetSelector: '[name="Lesson[title]"]' },
  { triggerSelector: '[for="Lesson_description"]', targetSelector: '[name="Lesson[description]"]' },
  { triggerSelector: '[for="mission-title"]', targetSelector: '[name="Mission[title]"]' },
  { triggerSelector: '[for="mission-description"]', targetSelector: '[name="Mission[description]"]' },
